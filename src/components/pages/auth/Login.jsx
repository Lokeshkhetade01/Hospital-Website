...
const Login = () => {
  ...
  // Redux state se loading aur error nikalna
  ...
  const handleSubmit = (e) => {
    ...
    // Login action dispatch karna
    dispatch(loginUser({ email, password })).then((result) => {
      // Agar backend se success true aata hai toh navigate karein
        ...
        <div className="bg-[#141415] border border-zinc-800/50 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button 
              type="submit"
              disabled={loading}
              className={`w-full py-4 ${loading ? 'bg-blue-600/50 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500'} text-zinc-100 font-bold rounded-2xl transition-all shadow-lg shadow-blue-900/20 flex items-center justify-center gap-2 group active:scale-[0.98]`}
            >
              {loading ? (
